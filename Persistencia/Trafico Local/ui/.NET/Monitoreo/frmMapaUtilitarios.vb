Public Class frmMapaUtilitarios

    Dim mapa As DataTable

    Private Sub frmMapaUtilitarios_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        On Error Resume Next
        Kill(My.Application.Info.DirectoryPath & "\mapautilitarios.htm")
        Call Cabecera()
        Call Marca()
        Call Cerrar()
        Utilitarios.Navigate(New System.Uri(My.Application.Info.DirectoryPath & "\mapautilitarios.htm"))
    End Sub

    Private Sub Cabecera()
        FileOpen(1, My.Application.Info.DirectoryPath & "\mapautilitarios.htm", OpenMode.Append)
        ...
        PrintLine(1, "<head>")
        PrintLine(1, "<title>Mapa de Ubicacion de Vehiculos Utilitarios</title>")
    ...
    Private Sub Marca()
        ...
        mapa = frmUbicaUtilitarios.dtMapa
        FileOpen(1, My.Application.Info.DirectoryPath & "\mapautilitarios.htm", OpenMode.Append)

        PrintLine(1, "function load() {")
        PrintLine(1, "var myOptions = { ")
        PrintLine(1, "center: new google.maps.LatLng(" & frmUbicaUtilitarios.latitud & "," & frmUbicaUtilitarios.longitud & "),")
        PrintLine(1, "zoom: 8,")
        ...
        For i As Integer = 0 To mapa.Rows.Count - 1
            PrintLine(1, "Viaje" & i.ToString() & " = new google.maps.Marker({")
            PrintLine(1, "position: new google.maps.LatLng(" & mapa.Rows(i).Item("latitude").ToString() & "," & mapa.Rows(i).Item("longitud").ToString() & "),")
            If frmUbicaUtilitarios.Detalle = True Then
                PrintLine(1, "icon:   'images/flecha.png',")
            Else
                PrintLine(1, "icon:   'images/tracto.png',")
            End If
            PrintLine(1, "map: map,")
            PrintLine(1, "title: '" & mapa.Rows(i).Item("unidad").ToString() & "'")
            PrintLine(1, "	});")

            PrintLine(1, "attachMessage(" & "Viaje" & i.ToString() & " , '" & mapa.Rows(i).Item("unidad").ToString() & "','" & mapa.Rows(i).Item("cliente").ToString() & "' );")
        Next
        FileClose()
    End Sub
